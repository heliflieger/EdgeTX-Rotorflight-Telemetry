...
function utils.edgeTXVersionAtLeast(targetVersion)
    local _, _, major, minor, revision = getVersion()
    rfsuite.log("Current EdgeTX version: %d.%d.%d", major, minor, revision)
    local currentVersion = {major, minor, revision}

    if targetVersion == nil then
        if rfsuite and rfsuite.config and rfsuite.config.edgeTXVersion then
            targetVersion = rfsuite.config.edgeTXVersion
        else

            return false
        end
    elseif type(targetVersion) == "number" then
        rfsuite.log("WARNING: utils.edgeTXVersionAtLeast() called with a number instead of a table (" .. targetVersion .. ")", 2)
        return false
    end
