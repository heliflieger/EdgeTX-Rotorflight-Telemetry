...
function ini.save_ini_file(fileName, data)
    ...
    -- To work around a suspected bug in the EdgeTX io.write implementation when called in a loop,
    -- we build the entire file content in a table and write it in a single operation.
    local content = {}
    for section, params in pairs(data) do
        table.insert(content, "[" .. tostring(section) .. "]\n")
    ...
    io.write(file, table.concat(content))
    io.close(file)
    return true
end
